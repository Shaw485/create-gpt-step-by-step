...
训练候选的互斥风险分层为：683条无自动风险标记，124条优先复核证据，1593条优先复核语义或任务改写。600条val/test无论自动标记如何，都属于决定发布门槛的P0真人审核对象。“无自动风险标记”只表示机器未发现已知问题，不表示事实已经过人工认证。

600条评估记录又经过一轮AI辅助预审。修复60条原因题语病并纠正2条“尚未达到二星”却答成“已经二星”的事实错误后，剩余22条应先修复再交给人审，250条需核验全书首次/最多等断言，138条需检查自动任务改写，190条进入低风险人工确认。该预审只负责排序，不产生批准状态。
...
1. 先修复600条val/test中的22条阻塞项，再核验250条全局或聚合断言。
2. 审核138条任务改写和190条低风险记录，并填写真实的决定、审核人和审核时间。
3. 再处理训练集124条证据风险与1593条语义/任务改写风险；无法确认的记录删除或改成诚实未知任务。
