...
600条评估记录又经过一轮AI辅助预审。修复60条原因题语病并纠正2条“尚未达到二星”却答成“已经二星”的事实错误后，又将22条评估集旧章节号重绑定到冻结v4语料。全数据共137条旧章节号完成同步修正，并保留原编号与原标题供追溯。评估集已无自动阻塞项，剩余255条需核验全书首次/最多等断言，148条需检查自动任务改写，197条进入低风险人工确认。该预审只负责排序，不产生批准状态。
...
1. 核验255条全局或聚合断言，使用全书首次出现索引和完整章节计数，不能只看一条证据。
2. 审核148条任务改写和197条低风险记录，并填写真实的决定、审核人和审核时间。
