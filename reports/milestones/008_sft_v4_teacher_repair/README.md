...
训练候选的互斥风险分层为：708条无自动风险标记，9条优先复核证据，1683条优先复核语义或任务改写。600条val/test无论自动标记如何，都属于决定发布门槛的P0真人审核对象。“无自动风险标记”只表示机器未发现已知问题，不表示事实已经过人工认证。

600条评估记录又经过一轮AI辅助预审。修复60条原因题语病并纠正2条“尚未达到二星”却答成“已经二星”的事实错误后，又将22条评估集旧章节号重绑定到冻结v4语料。全数据共137条旧章节号完成同步修正，并保留原编号与原标题供追溯。最初标记的255条全局/聚合断言中，38条是复制、澄清等任务被关键词规则误报；修正规则后，评估集分为217条真实全局核验、185条任务改写检查和198条低风险人工确认。该预审只负责排序，不产生批准状态。

217条真实全局断言已使用1775个保留章节段（1614个唯一章节号）建立全书字面索引。结果为78条章节/先后顺序与索引一致，56条目标人物在所有保留版本中的字面名字计数并列最高，44条章节断言与索引冲突，2条计数断言冲突，4条先后结论正确但答案章节号过期，另有33条涉及“主要人物”或“无法判断”等语义问题，不能用字面索引自动证明。即使机器支持的134条也仍需真人确认，因为同章出现不等于同场互动，名字次数不等于戏份，别名、代词和隐含指代也不在字面索引中。
...
- `global_claim_verification.json`：217条全局断言的逐条索引结果。
...
1. 人工处理44条章节冲突、2条计数冲突和4条章节号过期记录；不能直接把字面“同章”改写成语义“同框”。
2. 人工确认134条机器支持项并判断33条纯语义项，再审核185条任务改写和198条低风险记录，填写真实的决定、审核人和审核时间。
3. 再处理训练集9条证据风险与1683条语义/任务改写风险；无法确认的记录删除或改成诚实未知任务。
