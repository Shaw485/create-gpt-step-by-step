...
随后22条评估集旧章节标注全部重绑定到冻结v4语料，完整数据共修正137条，并保留旧编号和标题作为审计轨迹。最初255条全局标记中有38条属于关键词规则误报，实际217条进入全书索引；索引发现134条机器支持、46条冲突、4条章节号过期和33条纯语义待审。这个结果适合作为视频中的数据治理案例：局部证据存在不能证明“首次、最多、主要人物”，而字面同章也不能证明真正同框。
