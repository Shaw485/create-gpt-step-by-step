...
M011回应了M010里“怎么全是数字、一点都不聊天”的问题：在2999条小说SFT之外加入3000条通用聊天、数学、项目解释、实时能力边界、学习计划、指令改写、诚实未知和领域切换样本，合计5999条重新编码训练。2000步后验证Loss降到1.6600，模型开始能对未知实体说资料不足，也会对天气问题给出能力边界，而不是一味回答章节号。与此同时，固定样本仍显示数学、人物介绍和部分解释明显不可靠。视频中可以把这一阶段作为“数据分布改变模型行为，但不是魔法”的案例：SFT先学回答格式，再逐步逼近可靠内容。
