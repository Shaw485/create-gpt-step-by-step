...
| M008 | 3000条教师SFT候选的保守修复与审计 | 待人工验收 | [结构指标、风险分层与复核协议](reports/milestones/008_sft_v4_teacher_repair/README.md) |
...
M008直接修复而非重新生成3000条教师数据，保留了生成成本并完成结构、切分、证据和分词审计。第一次机器门槛通过后，内容抽样仍发现截断复制、伪一句话和不自然澄清，因此进行了第二轮修复；证据定位提高到2999/3000，最长序列降到149 Token。继续修复原因题语病和2条修为事实错误后，600条评估记录仍未真人批准，AI预审只负责把它们分成22/250/138/190四档。视频中应保留这一“机器门槛通过不等于高质量训练集”的阶段，人工验收后再进入20步SFT安全试跑。

随后22条评估集旧章节标注全部重绑定到冻结v4语料，完整数据共修正137条，并保留旧编号和标题作为审计轨迹。评估集的自动阻塞项降为0，但255条“首次/最多/主要人物”等断言仍需全书或全章核验，不能因为局部证据存在就宣布正确。
