...
进一步核验发现冻结语料同时保留正式分隔符标题和少量旧版内嵌标题，旧定位器误把两者都当成章节边界。统一采用正式v4标题后，完整数据累计重绑定262条旧章节号。另有271条“首次同框”只能由局部证据证明两个人名同时出现，因此改成局部证据理解或诚实未知任务。最终172条真实全局断言中，137条获字面索引支持，35条保留语义人审，自动冲突为0。这个过程适合作为视频中的数据治理案例：局部证据存在不能证明“首次、最多、主要人物”，发现指标口径错误后必须回到源数据重算。
